...
def main():
    ...
    for i, model in enumerate(models):
        ...
        for layer_name, attn_data in attn_patterns.items():
            ...
            per_head_cos = 1 - abs(F.cosine_similarity(neg_side, pos_side, dim=-1))  # shape: [H]
    ...
    k = "cosine" #, "entropy", "sum_to_last", "sum", "max" euclidean cosine
    v = "max" # "max" "mean"
# '''below is for attention patterns visualization, currently commented out'''
    ...
    for model_name, attn_dict in attention_by_model.items():
        ...
        plt.subplot(8, 4, iterator)
        plt.title(f"{k} Dist (N, P), {model_name.split('/')[-1]}")
        plt.xlabel("Head ID")
        plt.ylabel("Layer ID")
        plt.xticks(rotation=90)
        plt.yticks(rotation=0)
        plt.imshow(np.array(entropy_diff), aspect='auto', cmap='coolwarm', vmin=-1, vmax=1)
        plt.colorbar(label=f'{k} distance')

        plt.subplot(8, 4, iterator + 1)
    ...
    plt.tight_layout()
    plt.savefig(f"{k}_distance_across_models.png", bbox_inches='tight')
