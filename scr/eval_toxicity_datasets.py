...
def parse_args():
    ...
    p.add_argument("--num_prompts", type=int, default=3000)
...
def main(args):
    ...
    if os.path.exists(os.path.join(args.output_dir, f"{safe_model_name}/eval_toxicity_{safe_dataset}.csv")):
        df = pd.read_csv(os.path.join(args.output_dir, f"{safe_model_name}/eval_toxicity_{safe_dataset}.csv"), sep=";")
        ...
        out_file = os.path.join(
            args.output_dir, f"{safe_model_name}/eval_toxicity_{safe_dataset}.csv"
        )
        ...
        np.save(f"{args.output_dir}/{safe_model_name}/labels_{safe_dataset}.npy", labels_f)
